    ...
    <NavBar/>
        <div className="container mx-auto mt-8">
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    {transactions.map(transaction => (
                        <tr key={transaction.id}>
                            ...
                            <td className={`px-6 py-4 whitespace-nowrap text-right ${transaction.amount < 0 ? 'text-red-500' : 'text-green-500'}`}>
            ...
            <div className="bg-white shadow-lg rounded-lg p-4">
                <h2 className="text-xl font-semibold mb-2 text-right">Aktuelles Guthaben</h2>
                <p className="text-gray-600 text-right">Das aktuelle Guthaben beträgt: <span
                    className={`font-bold ${balanceClass}`}>
                    {balanceSign}{Math.abs(balance)} €</span></p>
            </div>
        </div>
</ProtectedRoute>
    );
